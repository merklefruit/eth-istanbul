...
contract RebalanceTest is BaseComposableCoWTest {
    ...
    function mockCowCabinet(
        address mock,
        address owner,
        bytes32 ctx,
        bytes32 retVal
    ) internal returns (ComposableCoW iface) {
        iface = ComposableCoW(mock);
        vm.mockCall(
            mock,
            abi.encodeWithSelector(iface.cabinet.selector, owner, ctx),
            abi.encode(retVal)
        );
    }
    ...
    function test_pricing_LimitPriceAtStart_concrete() public {
        ...
        GPv2Order.Data memory res = dutchAuction.getTradeableOrder(
            safe,
            address(0),
            bytes32(0),
            abi.encode(data),
    ...
    function test_verifyOrder() public {
        ...
        GPv2Order.Data memory order = dutchAuction.getTradeableOrder(
            safe,
            address(0),
            bytes32(0),
            abi.encode(data),
    ...
    function test_timing_RevertBeforeAuctionStarted() public {
        ...
        dutchAuction.getTradeableOrder(
            safe,
            address(0),
            bytes32(0),
            abi.encode(data),
    ...
    function test_e2e_settle() public {
        ...
        (GPv2Order.Data memory order, bytes memory sig) = composableCow
    ...
    function helper_runRevertingValidate(
        DutchAuction.Data memory data,
    ...
    function helper_miningTime()
        internal
        returns (DutchAuction.Data memory, bytes32, DutchAuction, uint256)
    {
        DutchAuction.Data memory data = helper_testData();
        data.startTime = 0;
        ...
        DutchAuction auction = new DutchAuction(
            mockCowCabinet(
                ...
                safe,
                id,
                bytes32(uint256(startTime))
            )
        );
        return (data, id, auction, startTime);
    }

    function helper_testData()
        internal
        view
        returns (DutchAuction.Data memory data)
    {
        return
            DutchAuction.Data({
                sellToken: token0,
